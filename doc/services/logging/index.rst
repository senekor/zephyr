...
Custom frontends and backends can be used if needed.
TODO: What is the function of a frontend and a backend? I assume it is explained later, but it would
be great to have a one-sentence summary here already. example:
Frontends control how logs are formatted, whereas backends represent a destination where logs are
sent or stored.
...
- Custom frontend support. It can work together with backends. What?
...
- Support for logging floating point and long long arguments.
...
TODO: What is an "instance" of a module?
...
- ``LOG_INST_X`` for standard printf-like message associated with a
...
severity level. A standard message contains pointers (TODO: plural correct?) to the string and arguments.
...
a feature which will wake up the processing thread when a certain amount of log messages is
...
TODO: question about the aggregate filter slot. there is only one. how can it apply
      "to a given source of logging" ? won't it apply to all sources equally?
...
* A compiler with C11 ``_Generic`` keyword support is recommended. Logging
